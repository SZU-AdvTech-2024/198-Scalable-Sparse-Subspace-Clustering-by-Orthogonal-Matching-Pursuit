...
def active_support_elastic_net(X, y, alpha, tau=1.0, algorithm='spams', support_init='knn',
    ...
    algorithm : string, default ``spams``
        Algorithm for computing solving the sub-problems. Either lasso_lars or lasso_cd or spams
...
def elastic_net_subspace_clustering(X, gamma=50.0, gamma_nz=True, tau=1.0, algorithm='lasso_lars',
    ...
    n_nonzero : int, default 50
        ...
        entries with the largest absolute value are kept.
...
class ElasticNetSubspaceClustering(SelfRepresentation):
    ...
    def __init__(self, tau, n_clusters=8, affinity='symmetrize', random_state=None, n_init=20, n_jobs=1, gamma=50.0,
                 gamma_nz=True,
                 algorithm='spams', active_support=True, active_support_params=None, n_nonzero=50):
        self.gamma = gamma
...
def sparse_subspace_clustering_orthogonal_matching_pursuit(X, n_nonzero=10, thr=1.0e-6):
    ...
    # affinity = sparse.csr_matrix((vals, (rows, cols)), shape=(n_samples, n_samples)) + sparse.csr_matrix((vals,
    # (cols, rows)), shape=(n_samples, n_samples))
    return sparse.csr_matrix((vals, (rows, cols)), shape=(n_samples, n_samples))
...
def least_squares_subspace_clustering(X, gamma=10.0, exclude_self=False):
    """Least squares subspace clustering.(LSR)
    Compute self-representation matrix C by solving the following optimization problem
...
class LeastSquaresSubspaceClustering(SelfRepresentation):
    ...
    def __init__(self, n_clusters=8, affinity='symmetrize', random_state=None, n_init=10, n_jobs=1, gamma=10.0,
                 exclude_self=False):
        self.gamma = gamma
