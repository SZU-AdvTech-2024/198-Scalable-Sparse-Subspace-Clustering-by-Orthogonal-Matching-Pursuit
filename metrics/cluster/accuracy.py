...
def self_representation_loss(labels_true, representation_matrix):
    ...
    for i in range(n_samples):
        representation_vec = np.abs(representation_matrix[i, :]).reshape(1, -1)
...
def self_representation_sparsity(representation_matrix):
    ...
    """
    nonzero = representation_matrix.count_nonzero()
    shapes = representation_matrix.shape[0]
    return nonzero / shapes
...
def self_representation_connectivity(labels_true, representation_matrix):
    ...
    for i in np.unique(labels_true):
        ...
        try:
            class_affinity_matrix_ = 0.5 * (
                    np.absolute(class_representation_matrix) + np.absolute(class_representation_matrix.T))
